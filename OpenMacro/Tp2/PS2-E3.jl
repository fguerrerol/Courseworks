...
include("itpcake.jl")
cd("/home/francisco/Escritorio/Open-macro/Ramiro/")
...
PS2_graph3 = plot(scatter(x = (maximum(ce.kgrid) .- ce.gc), y = ratio_kk), Layout(title = "Ratio k/k en función de k"))
...
PS2_graph4 = plot([p1,p2], Layout(title = "Consumo y ahorro en función del capital"))
...
savefig(PS2_p2, "Diferentes niveles de ingreso.png")
...
function simul_ingresos(;T::Number=40)
    ...
    itp_c = interpolate(knots, ingresos.gc, Gridded(Linear()));
    itp_k = interpolate(knots, ingresos.gk, Gridded(Linear()));
    y_index=0
    ...
    for i in Time 
        if i == 1
            y_index = 13;
            k_0 = maximum(ingresos.kgrid)                   
        else
            y_index = sample(Ny,Weights(ingresos.Py[:,y_index]))
        end
        c = itp_c(k_0,ingresos.ygrid[y_index])
        k_1 = itp_k(k_0,ingresos.ygrid[y_index]) 
        k_0= k_1;
        C[i] = c;
        K[i] = k_1/(1+ce.r);
        Y[i] = ce.ygrid[y_index];
    ...
    plot([plot_consumo, plot_ahorro,plot_dotacion], Layout(title="Consumo y Ahorro restante", xaxis_title="Tiempo"))
    global Grafo = Elementos(C,Y,K);
    
    p1= plot([histogram(x = Grafo.C./Grafo.Y, opacity=0.75)], Layout(title="Distribución de c/y"))
...
A_graph = plot([scatter(x = A.bgrid, y = A.prob[:,jy],name= "y = $(A.ygrid[jy])") for jy in eachindex(A.ygrid)])
B_graph = plot([scatter(x = B.bgrid, y = B.prob[:,jy],name= "y = $(B.ygrid[jy])") for jy in eachindex(B.ygrid)])


C_graph = plot([scatter(x = C.bgrid, y = C.prob[:,jy],name= "y = $(C.ygrid[jy])") for jy in eachindex(C.ygrid)])

D_graph = plot([scatter(x = C.bgrid, y = C.prob[:,jy],name= "y = $(C.ygrid[jy])") for jy in eachindex(D.ygrid)])
...
savefig(A_graph, "Mayor factor de descuento.png")
savefig(B_graph, "Parametros normales.png")
